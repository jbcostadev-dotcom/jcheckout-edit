...
setTimeout(function() {
    ...
    setTimeout(function() {
        ...
        if (progressBar2) {
            ...
            setTimeout(function() {
                progressBar2.style.transition = 'width 8s linear'; // Define a duração da animação para 15 segundos
                progressBar2.style.width = '100%'; // Preenche a barra até 100% em 15 segundos
            }, 100); // Pequeno atraso para garantir que a transição seja visível
        }
    }, 4500); // Após 4.5 segundos

    setTimeout(function() {
        document.getElementById('box-2').classList.add('hidden2');
        document.getElementById('box-3').classList.remove('hidden');
    }, 12500); // Após mais 5 segundos (total 10 segundos)

    setTimeout(function() {
        document.getElementById('box-rastreio').classList.remove('hidden');
        smoothScrollToBox('box-3'); // Aplica o efeito de scroll extra suave apenas para a terceira box
    }, 15000); // Após 13 segundos
