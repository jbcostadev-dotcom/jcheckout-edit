...
if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
    ...
    if (($handle = fopen($filename, "r")) !== false) {
        ...
        fclose($handle);
        header("Location: index.php?columns=" . implode(",", $columns) . "&filename=" . $filename);
        exit();
    }
}
...
if (isset($_POST['import']) && isset($_POST['filename'])) {
    ...
    if (($handle = fopen($filename, "r")) !== false) {
        ...
        $sql = "INSERT INTO lead_cadastro (cpf, nome, CEP, cidade, estado, rua, numero, data_compra, produto_preco, bairro, celular, produtos, link_checkout, categoria)
                VALUES (:cpf, :nome, :CEP, :cidade, :estado, :rua, :numero, :data_compra, :produto_preco, :bairro, :celular, :produtos, :link_checkout, :categoria)";
